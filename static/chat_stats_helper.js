      ...
      if (data.stats) {
        ...
        .then(data => {
          ...
          if (msgDiv && data.reply) {
            ...
            msgDiv.textContent = 'Error: Gemini did not return a reply.';
          } else {
            const errorDiv = document.createElement('div');
            errorDiv.className = 'msg gemini';
            errorDiv.textContent = 'Error: Gemini did not return a reply.';
            chatBox.appendChild(errorDiv);
          }
        })
        .catch(() => {
          let geminiMsg = chatBox.querySelector('.msg.gemini:last-child');
          if (geminiMsg) {
            geminiMsg.textContent = 'Error: Could not reach Gemini.';
          } else {
            // Fallback: append a new Gemini message
            const msgDiv = document.createElement('div');
            msgDiv.className = 'msg gemini';
            msgDiv.textContent = 'Error: Could not reach Gemini.';
